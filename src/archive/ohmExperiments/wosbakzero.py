    ...
    def __call__(self, module):
        ...
        if hasattr(module, 'weight'):
            ...
            for fi in range(weight.shape[0]):
                ...
                w[w > ZERO_TOL] = WW
                t = TT
                weight[fi] = torch.tensor(w)
                bias[fi] = torch.tensor(t)
                #print("IWOS!!!")
                #print(w)
                #rint(t)
...
class OHMWosFunction(Function):
    ...
    def forward(ctx, input, mask, weight, bias):        
        ...
        ctx.nzw = weight > ZERO_TOL
        lweight = weight[ctx.nzw]
        lmx = mx[ctx.nzw]
        ...
        smx, si = torch.sort(lmx, 2, descending=True)
        ...
        for ci in range(NC):
            ...
            yl = torch.gather(mysmx, 1, lici[:,ci].unsqueeze(-1))
            y[:,ci] = yl.squeeze()
